...
function setupInputFieldListeners() {
    const globalRelationshipIDField = document.querySelector('input[name="Global_Relationship_ID"]');
    const metaIDField = document.querySelector('input[name="Meta_ID"]');
    const uniqueIDField = document.querySelector('input[name="Unique_ID"]'); // Add this line
    const jointlyReportedUniqueIDField = document.querySelector('input[name="Jointly_Reported_Unique_ID"]'); // Add this line

    const logAndSendData = () => {
        console.log('Input field changed. Triggering data send.');
        sendGlobalIDSessionToServer();
    };

    if (globalRelationshipIDField) {
        globalRelationshipIDField.addEventListener('change', logAndSendData);
    }

    if (metaIDField) {
        metaIDField.addEventListener('change', logAndSendData);
    }

    // Add event listeners for Unique_ID and Jointly_Reported_Unique_ID fields
    if (uniqueIDField) {
        uniqueIDField.addEventListener('change', logAndSendData);
    }

    if (jointlyReportedUniqueIDField) {
        jointlyReportedUniqueIDField.addEventListener('change', logAndSendData);
    }
}
...
// THE BELOW MIGHT BE OLD CODE. I HAVEN'T YET FOUND A BUG WHEN REMOVING IT BUT STILL AM TESTING
