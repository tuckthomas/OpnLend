function initializeFeeSchedule() {
    const container = document.querySelector('.fee-schedule-container');

    // Function to add a new row
    function addRow() {
        const tbody = container.querySelector('table tbody');
        const newRow = tbody.rows[0].cloneNode(true); // Clone the first row
        newRow.querySelector('input[field-name^="Fee_Description"]').value = ''; // Clear the input values for cloned row
        newRow.querySelector('input[field-name^="Fee_Amount"]').value = ''; // Clear the input values for cloned row
        newRow.querySelector('input[field-name^="Fee_Description"]').setAttribute('field-name', `Fee_Description_${tbody.rows.length + 1}`);
        newRow.querySelector('input[field-name^="Fee_Amount"]').setAttribute('field-name', `Fee_Amount_${tbody.rows.length + 1}`);
        tbody.appendChild(newRow);
        updateTotal(); // Update the total after adding a new row
    }

    // Function to update the fee total
    function updateTotal() {
        const feeInputs = container.querySelectorAll('input.Fee-Amount-Input');
        const total = Array.from(feeInputs).reduce((acc, input) => acc + (parseFloat(input.value) || 0), 0);
        container.querySelector('tfoot .fee-amount').textContent = total.toFixed(2);
    }

    // Attach event listener to the add row button
    container.querySelector('.Fee-Add-Row-Button').addEventListener('click', addRow);

    // Event delegation for delete buttons
    container.addEventListener('click', function(e) {
        if (e.target.closest('.del-row')) {
            const row = e.target.closest('.sources-and-uses-row');
            row.parentNode.removeChild(row); // Remove the row
            updateTotal(); // Update the total after deleting a row
        }
    });

    // Event listener for updating the total when fee amounts change
...
function setupTextFields() {
    ...
    textFields.forEach(field => {
        ...
        if (['numeric-whole', 'numeric', 'currency', 'percent'].includes(format)) {
            ...
            if (format === 'currency') {
                // Handle onBlur for currency to format after input is complete
                field.addEventListener('blur', function() {
                    let value = parseFloat(this.value.replace(/[^0-9.]/g, ''));
                    if (!isNaN(value)) {
                        this.value = value.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
                        this.value = `$${this.value}`;
                    }
                });
            } else {
                // Handle onInput for numeric and percent formats
                field.addEventListener('input', function() {
                    this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1'); // Allow only numbers and a single decimal point

                    if (format === 'percent' && !this.value.includes('%')) {
                        // Append '%' for percent but avoid immediate formatting to not disrupt typing
                    }
                });
            }
        }

        // Simplified handling for numeric-whole, as this does not change
        if (format === 'numeric-whole') {
            field.addEventListener('input', function() {
